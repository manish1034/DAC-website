...
const About = () => {
  ...
  useEffect(() => {
    const fetchData = async() => {
      try{
        const response = await axios.get("https://dac-website-backend.onrender.com/api/team",{ 
          headers: {
        ...
        {data.length > 0 ? (
          <>
            {data.filter(item => item.name === 'Vikas Pandey').map((item, index)=>(
              <div key={index} className="flex flex-col h-[34rem] w-[25rem] bg-[#dfe8f6] dark:bg-[#1a222f] rounded-xl mt-10 ml-[10rem] justify-start shadow-md">
                <img src={item.image} alt="" className="h-[9rem] w-[9rem] ml-[8rem] rounded-full mt-10 text-[2rem] text-[#0177b5] cursor-pointer"/>
                <h1 className="text-center text-black dark:text-white text-4xl mt-3 font-bold">{item.name}</h1>
